...
SCRIPT='/software/public/qsub_sge/send_email.py'
...
def scp_db_file( user , old_db , full_flag , retry_time):
	try:
		ssh = paramiko.SSHClient()
		ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
		ssh.connect(IP , PORT , user )
		...
		if not myrun( ssh , "mkdir -p ~/{0} && echo ok".format(pid) ):
			sys.exit("目录创建失败")
		if not myrun(ssh , "scp {0}@{1}:{2} ~/{3}/  && echo ok ".format(user , IP2, old_db , pid)):
			sys.exit("拷贝失败")
		if full_flag:
			if not myrun( ssh  , "{0}  {1} -i ~/{3}/{2} -f -m {4}    && echo ok ".format(PYTHON , SCRIPT , os.path.basename(old_db) ,pid , retry_time )):
				sys.exit("发送邮件失败")
		else:
			if not myrun( ssh ,  "{0}  {1} -i ~/{3}/{2} -m {4}  && echo ok ".format(PYTHON , SCRIPT , os.path.basename(old_db) ,pid , retry_time )):
				sys.exit("发送邮件失败")
...
''')
		...
		print("发信配置成功")
	scp_db_file(user , os.path.abspath(args.input) , args.full ,args.retry)
