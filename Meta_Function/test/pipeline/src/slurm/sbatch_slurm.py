...
CPU_INCREASE_RATIO = 1.8
MEMORY_CPU_RATIO = 3.6
...
class Script():
	...
	def qsub(self, f_out ):
		...
		memory = self.uvf if self.uvf[-1].isdigit() else int(self.uvf[:-1]) 
		#debug_log.info(' memory {0} up {1} uvf {2}'.format(memory , self.up , self.uvf))
		if memory / self.up > MEMORY_CPU_RATIO :
			self.up = int(memory / MEMORY_CPU_RATIO ) + 1
		debug_log.info('memory {0} up {1}'.format(memory , self.up))
		cmd = 'sbatch -J {0.name} -N 1 -n {0.up} -p {0.queue} --mem {0.uvf}  -D {1} -o {2}.o%j -e {2}.e%j {0.script}'.format(self , os.path.abspath(os.path.dirname(self.script)) , os.path.basename(self.script))
		debug_log.info(cmd)
		while count <= 5 :
			qsub=''
			try:
				qsub = "".join(popen(cmd , 3))
			except SystemExit :
				pass
			if qsub.startswith('Submitted batch job'):
				self.jobid = qsub.split()[3]
				self.state = 'qsub'
				self.qsub_time += 1
				return self.jobid 
			elif qsub.startswith('sbatch: error:'):
				time.sleep(60)
			else:
				time.sleep(60)
			count += 1
			...
			debug_log.error("Exit :  sbatch failed {0}".format(cmd))
			sys.exit(1)
	def qsub_to_run(self):
		self.time_last = time_current() #time.time()
		self.state = 'running'
	def qhold(self):
		cmd = 'scontrol suspend  {0.jobid}'.format(self)
		if os.system(cmd):
			debug_log.warning('scontrol suspend {0} {1} fail'.format(self.name , self.jobid ))
		else:
			self.state = 'hold'
	...
	def qrelease(self , db):
		...
		if len(qstat) > 0 : 
			cmd = 'scontrol resume {0.jobid}'.format(self)
			if os.system(cmd):
				debug_log.warning('scontrol resume {0} {1} fail'.format(self.name , self.jobid ))
			else:
				self.state = 'running'
			...
			self.job_obj.qrls_time = time_current() #time.time()
	def qdel(self , db):
		cmd = ' scancel {0.jobid}'.format(self)

		def update_vf(ratio , old):
			print('old',old , type(old))
			old = str(old)
			if old[-1].isdigit():
				old = float(old)
				return int(old*ratio)+1
			else:
				danwei = old[-1]
				old = float(old[:-1])
				new = int(old * ratio) + 1
				return '{0}{1}'.format(int(new) , danwei)

		#self.up = self.up if self.up > int(self.job_obj.uthread + 1 ) else int(self.job_obj.uthread + 1)
		self.up = update_vf(CPU_INCREASE_RATIO , self.up)
		self.uvf =update_vf(MEMORY_INCREASE_RATIO  , self.uvf)

		memory = int(self.uvf) if self.uvf[-1].isdigit() else int(self.uvf[:-1]) 
		if memory / self.up > MEMORY_CPU_RATIO :
			self.up = int(memory / MEMORY_CPU_RATIO ) + 1 

		self.uh_vmem = update_vf(hvmem_ratio  , self.uvf)
		r_dict = dict(zip(['OLDP' ,'OLDVF','OLDHVMEM'],[self.oldp,self.oldvf,self.oldh_vmem]))
		db.update('SCRIPT' , r_dict , 'ID', self.count)
		if os.system(cmd):
			debug_log.warning('WARNING : qdel {0.jobid} failed'.format(self))
		else:
			self.state = 'break'
	def __str__(self):
...
def popen(cmd , max_count = 10  ):
	...
	while count < max_count :
		...
		#child.wait()
			...
			#print('{0} is error\n'.format(cmd))
			debug_log.warning('{0} failed , repeat {1} time'.format(cmd , count ))
...
def makedir(dir):
	if not os.path.isdir(dir):
		...
		debug_log.info('{0} is exist'.format(dir))
		if os.system('rm -rf {0}/*'.format(dir)):
			debug_log.error("Exit : cannot rm -rf {0}/*".format(dir))
			sys.exit("cannot rm -rf {0}/*".format(dir))
	...
	with open(shell) as f_in:
		for content in f_in:
			...
			if line_count % line_interval == 0 :
				...
				shell_out.write('#!/bin/sh\n')
				if args.singularity:
					shell_out.write('module load singularity/3.7.3\n')
			...
			if args.singularity:
				...
				shell_out.write('singularity exec --bind {1} {2} {0} &&\\\n'.format(new_content , args.mount , args.singularity))
			else:
				shell_out.write('{0} &&\\\n'.format(content))
			line_count += 1
		else:
	...
	if flag_nodu: 
		...
		if size >0.95*qq:
			try:
				...
				popen(' ssh c0008 /annoroad/share/software/install/Python-3.3.2/bin/python3 {0}/jump_send_email.py -i {1}.db -full'.format( bindir, os.path.abspath(input)))
				if os.path.isfile('{0}.db.ini'.format( input)):
...
def check_o_file(a_script):
	print('{0.script}.o{0.jobid}'.format(a_script))
	if os.path.isfile('{0.script}.o{0.jobid}'.format(a_script)):
		content = ''
		with open('{0.script}.o{0.jobid}'.format(a_script)) as f_in:
			for i in f_in:
				content += i
		print(content)
		if content.rstrip().endswith('This-Work-is-Completed!'):
			return True
		else:
			return False
	else:
...
def check_e_file(a_script):
	if os.path.isfile('{0.script}.e{0.jobid}'.format(a_script)):
		content = ''
		with open('{0.script}.e{0.jobid}'.format(a_script)) as f_in:
			for i in f_in:
				content += i
		print(content)
		if content.rstrip().endswith('MemoryError'):
			return True
		else:
			return False
	else:
...
def guard_objs(obj_dict , args ,logfile , db , db_obj):
	...
	while(all_script_list):
		...
		if len(dead_node) : 
			debug_log.warning('dead node : {0}'.format(' '.join(dead_node)))
					...
					if len(qstat) > 0 :
						...
						if a_job.state == 'r': 
							...
							if a_job.node in dead_node:
								debug_log.warning('WARNING : job {0} on dead node {1}'.format(a_job.jobid , dead_node))
							#elif a_job.slow_node() :
							#	debug_log.warning( 'WARNING :job {0} on slow node {1}'.format( a_job.jobid , a_job.slow_node() ))
							#elif a_job.exceed_resource()[0] > args.kill_cpu:
							#	debug_log.warning( 'WARNING:job {0} exceed cpu {1}'.format( a_job.jobid , a_job.exceed_resource()[0]  ))
							#elif a_job.exceed_resource()[1] >args.kill_mem :
							#	debug_log.warning(' WARNING:job {0} exceed mem {1}'.format( a_job.jobid, a_job.exceed_resource()[1] ))
							else :
							...
							#print(a_job.__str__())
							debug_log.warning('WARNING : {0} was qdel'.format(a_job.jobid))
							a_script.qdel(db_obj)
							a_job.end = time_current()
							a_job.kill = 1
							count_running_job -= 1
						...
						if bool_finish:
							...
							if bool_memory_error:
								...
								a_script.uvf = increase_resource(MEMORY_INCREASE_RATIO , a_script.uvf) 
								a_script.uh_vmem = a_script.uvf
...
def increase_resource(ratio , vf):
	print("increase resource {0} {1}".format(ratio , vf))
	if vf.endswith("G") or vf.endswith("g"):
		vf = float(vf.rstrip("Gg"))
		vf = str(int(vf * ratio) +1 ) + "G"
	elif vf.endswith("M") or vf.endswith("m"):
		vf = float(vf.rstrip("Mm"))
		vf = str(int(vf * ratio)+1 ) + "M"
	elif vf.endswith("K") or vf.endswith("k"):
		vf = float(vf.rstrip("Kk"))
		vf = str(int(vf * ratio)+1) + "K"
	else:
		vf = str(int(float(vf) * ratio))
	return vf
...
def transfer(count):
	c_maxvmem = 0
	if count.endswith('T'):
		c_maxvmem = int(count.replace('T', '')) * 1024
		c_maxvmem = f'{c_maxvmem}G'
	elif count.endswith('t'):
		c_maxvmem = int(count.replace('t', '')) * 1024
		c_maxvmem = f'{c_maxvmem}G'
	elif count.endswith('G'):
		c_maxvmem = int(count.replace('G', '')) 
		c_maxvmem = f'{c_maxvmem}G'
	elif count.endswith('g'):
		c_maxvmem = int(count.replace('g', ''))
		c_maxvmem = f'{c_maxvmem}G'
	elif count.endswith('M'):
		c_maxvmem = float(count.replace('M', ''))
		c_maxvmem = f'{c_maxvmem}M'
	elif count.endswith('m'):
		c_maxvmem = float(count.replace('m', ''))
		c_maxvmem = f'{c_maxvmem}M'
	elif count.endswith('K'):
		c_maxvmem = float(count.replace('K', ''))
		c_maxvmem = f'{c_maxvmem}K'
	elif count.endswith('k'):
		c_maxvmem = float(count.replace('k', ''))
		c_maxvmem = f'{c_maxvmem}K'
	elif count == 'N/A':
...
def main():
	...
	parser.add_argument('-d','--database',  help='database , default is {0}/database/slurm.db'.format(os.environ['HOME']),dest='database', default='{0}/database/slurm.db'.format(os.environ['HOME']))
	...
	parser.add_argument('-mount','--mount',help='mount point , default is /',dest='mount',default='/')
	parser.add_argument('-s','--singularity' , help='sif file', dest='singularity' , type=str )
	...
	print(f"noreqsub:{args.noreqsub} nocontinue:{args.nocontinue} nodu:{args.nodu} analysis_dir:{args.analysis_dir}")
	...
	if not bool_exists:
		...
		for i in db_obj.query_all('SCRIPT'):
			...
			job_id = JOBID
			if (p , vf , h_vmem , other) == (P , VF , HVMEM, OTHER ):
				a_script.add_attrib(QUEUE , P , VF , HVMEM , OTHER , OLDP, OLDVF , OLDHVMEM,  args.max_cycle ,STATE , int(FINISH) , 0  , job_id , args.qsub_P)
			else:
	...
	try:
		...
		debug_log.info(' ssh c0008 2>/dev/null /annoroad/share/software/install/Python-3.3.2/bin/python3 {0}/jump_send_email.py -i {1}.db'.format( bindir, os.path.abspath(args.input)))
		popen(' ssh c0008 /annoroad/share/software/install/Python-3.3.2/bin/python3 {0}/jump_send_email.py -i {1}.db'.format( bindir, os.path.abspath(args.input)))
	except:
		pass
	'''
