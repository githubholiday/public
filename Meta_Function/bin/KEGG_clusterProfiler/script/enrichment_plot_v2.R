...
theme(axis.text.x=element_text(angle=45,vjust=1,hjust=1,size=7), #调整横纵标签旋转读书和字体大小等
...
p=p+scale_color_gradient(high = "#F4AEAE",low = "#428CCB")
p=p+scale_fill_gradient(high = "#F4AEAE",low = "#428CCB")
p=p+theme(plot.title=element_text(hjust=0.5))

#pdf(paste(opt$prefix, ".barplot.pdf", sep = ""), width=width_len, height=height_len)
pdf(paste(opt$prefix, ".barplot.pdf", sep = ""), width=5, height=3)
