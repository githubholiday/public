...
print_usage <- function(para=NULL){
  ...
  cat("infile format:
      ...
      *Log2FC不能为NA,lnf,-lnf值
      ...
      ===========================================================================
      type          enricher(ORA)和GSEA分析时候，输入文件的gene是SYMBOL/ENSEMBL/ENTREZID。
      ===========================================================================
...
#====================输入文件处理函数============================
de_genes<-function( gene_file, species, type){
    print("Start:输入文件处理...")
    data<-read.table( gene_file,header=T,sep='\t')
    ######################
...
GO_clusterProfiler1<-function(gene,geneList,prefix,species,ont){
    print(paste("Start:Go terms processing of ",ont))
...
ORA_plot<-function(x,database){
    print(paste("Start:ORA_plot of ", database))
...
GSEA_plot<-function(y0,sign,database){
    y<-y0
    print("绘制第一个ID的gsea图，作为示例")
    graph <- paste(prefix,database,'gseplot.example.pdf',sep='_')
    pdf(graph, w=16, h=8)
    ID<-y$ID[1]
    pdf_title<-paste("enrichmentScore = ", signif(y[ID,]$enrichmentScore, digits = 2), sep = "")
 
    p1<-gseaplot2(y, geneSetID =ID, pvalue_table=T,title= pdf_title)
    p2<-ridgeplot(y,showCategory=10)
    #20240229
    emap_y <- pairwise_termsim(y)
    p3<-emapplot(emap_y,showCategory=10)
    #p3<-emapplot(y,showCategory=10)
    p4<-upsetplot(y,showCategory=10)
    #p<-plot_grid(p1,p2,p3,p4,ncol=2,labels=c("A","B","C","D")) #将多个图拼在一起
    print(p1)
    dev.off()
    ...
    #print(y)
    #write.table(y,file=paste(prefix,sign,"tcf.xls",sep="_"),sep="\t", quote=FALSE, row.names=FALSE)
    res <- subset(y,p.adjust< cut)
    #res <- subset(y,p_val< cut)
    #如果显著的超过50个，只画前50个terms
    if (nrow(res) >50){
        if (database=='MSigDB'){
            res<-res0
        }else{
            res <- head(as.data.frame(y),50)
            }
        }

    print("给特定的基因集绘制图")
    ids = c("M1066", "M13856","M17129","M17743")
    print(ids)
    for (k in (1:length(ids))){
        i = ids[k]
        print(i)
        filename<-paste(outdir,"/",i,'.GSEA_score.pdf')
        print("plot GSEA_score.pdf")
        pdf(filename, w=12, h=8)
        main<-paste("enrichmentScore = ", signif(y[i,]$enrichmentScore, digits = 2), sep = "")
        p1<-gseaplot2(y0, geneSetID =i,pvalue_table=T,title=main)
        p<- p1
        print(p)
        dev.off()
    }
    ...
    if (nrow(res) >=2){
        ...
        main<-paste("enrichmentScore = ", signif(y[i,]$enrichmentScore, digits = 2), sep = "")
        p1<-gseaplot2(y0, geneSetID =i,pvalue_table=T,title=main)
        #p2<-gseaplot2(y, geneSetID =i,title=paste(res$ID[k],"pvalue=",res$p.adjust[k],sep='_'))
        p<- p1 #+p2
        print(p)
        dev.off()
        }
...
genes <- de_genes( infile, mapda, type)
gene <- genes$gene
...
if ("KEGG" %in% databaseList){
    ...
    result_dir = paste(outdir,"KEGG",sep='/')
    mkdirs(result_dir)
    setwd(result_dir)
    if(organism=='human'){
        species<-'hsa'
