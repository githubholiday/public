...
QC_cellchat_raw<-function(rds,species,group.by='celltype',celltypes=names(table(rds$celltype))){
	mydata<-list(data=rds@assays$RNA@layers$data,meta=rds@meta.data)
	...
	cellchat <- createCellChat(object = rds@assays$RNA@layers$data, meta = rds@meta.data, group.by = group.by) #指定细胞群分群label
	return(cellchat)
}
...
cellchat1 <- createCellChat(object = rds1@assays$RNA@layers$data, meta = rds1@meta.data, group.by = 'celltype')  
cellchat_1a <- Infer_cellchat(cellchat1,CellChatDB.use,thresh=0.05,thresh.p = 1,thresh.pc=0.1,min.cells = 10)

cellchat2 <-  createCellChat(object = rds2@assays$RNA@layers@data, meta = rds2@meta.data, group.by = 'celltype') 
