...
if(is.null(ini.list$Para$maxcellnum)){
	...
	groups<-unique(unlist(strsplit(ini.list$sample$sample1,split = "/",fixed=T)))
	#cell_name<-read.table(opt$cellname,header=T,sep='\t',comment.char='')
	...
	metadata<-metadata[which(metadata[,"orig.ident"] %in% groups & metadata[,ini.list$Para$seurat_clusters] %in% cells),]
	...
	if(All_cell < maxnum){
		...
		for (group in groups){
			group_cell <- metadata[which(metadata[,"orig.ident"] %in% group),]
			sub_num <- round(maxnum*(dim(group_cell)/All_cell))
			for (cell in cells){
				group_cell_type <- metadata[which(metadata[,"orig.ident"] %in% group & metadata[,ini.list$Para$seurat_clusters] %in% cell),]
