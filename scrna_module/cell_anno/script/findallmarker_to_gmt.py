...
def main():
	...
	for cluster in result:
		...
		print('{0}_avg_log2FC\t{0}_avg_log2FC\t{1}'.format(cluster,'\t'.join(top_n_gene_name1)),file=args.output)
		...
		print('{0}_pct_diff\t{0}_pct_diff\t{1}'.format(cluster,'\t'.join(top_n_gene_name2)),file=args.output)
		...
		print('{0}_overlap\t{0}_overlap\t{1}'.format(cluster,'\t'.join(top_n_gene_name_overlap)),file=args.output)
