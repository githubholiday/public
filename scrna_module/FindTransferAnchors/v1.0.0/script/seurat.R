...
gene_space <- rownames(tmp)
gene_RNA<- rownames(tmp1)
...
if(is.null(tmp1@reductions$umap)){
	...
	sce_scale <- RunPCA(sce_scale, npcs = as.numeric(20), verbose = FALSE)
	#sce_umap <- RunUMAP(sce_scale, reduction = "pca",dims = 1:as.numeric(ini.list$Para$RunUMAP_dim))
	sce_umap <- RunUMAP(sce_scale, reduction = "pca",dims = 1:as.numeri(20))
...
saveRDS(tmp1, paste(prefix, "rna_sct.rds", sep="_"))
#输出空间数据的umap图
...
p2 <- DimPlot(pancreas.query, reduction = "umap", label = F,group.by = "predicted.id",label.size = 4,pt.size = 0,raster=FALSE)+ggtitle("")#,cols=my36colors)+ggtitle("")
print(p2)
dev.off()
