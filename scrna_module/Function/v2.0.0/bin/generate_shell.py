...
def main():
	...
	file_num =0
	sin_cmd = '/public/software/apps/singularity/3.7.3/bin/singularity exec --bind /work/share/acuhtwkcu9/:/work/share/acuhtwkcu9/ /work/share/acuhtwkcu9/install/sif/clusterprofiler_v0.0.4.sif '
	with open( args.output, 'w') as outfile:
		for infile in args.input:
			file_num += 1
			infile_dir_path, infile_name, cmp_name = info_from_infile( infile )
			if args.action == 'go':
				go_cmd = sin_cmd+ 'make -f {BIN}/GO/go.mk de_report={infile} go_dir={infile_dir_path}/GO cmp={cmp_name} species={species} GO_Candidate GO_Enrich'.format(BIN=bindir, infile=infile, infile_dir_path=infile_dir_path, cmp_name=cmp_name, species=args.species)
				outfile.write(go_cmd+'\n')
			
			elif args.action == 'kegg' :
				kegg_cmd = sin_cmd+'make -f {BIN}/KEGG/kegg.mk de_report={infile} kegg_dir={infile_dir_path}/KEGG cmp={cmp_name} species={species} category={category} KEGG_Candidate KEGG_Enrich'.format(BIN=bindir, infile=infile, infile_dir_path=infile_dir_path, cmp_name=cmp_name, species=args.species, category=args.category)
