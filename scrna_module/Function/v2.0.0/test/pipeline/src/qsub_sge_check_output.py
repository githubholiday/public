...
class Command():
	...
	def run(self, timeout): ## https://stackoverflow.com/questions/1191374/using-module-subprocess-with-timeout
		...
		try :
			self.content , self.err  = self.process.communicate(timeout=timeout)
...
class Script():
	...
	def qsub(self, f_out):
		...
		if self.qsub_P  and self.qsub_P != 'none' :
			cmd = 'cd {2} && qsub -cwd -S /bin/sh -q {0.queue} -P {0.qsub_P} -l {1} {0.script}'.format(self , self.resource() ,  os.path.abspath(os.path.dirname(self.script)))
		else:
			cmd = 'cd {2} && qsub -cwd -S /bin/sh -q {0.queue}  -l {1} {0.script}'.format(self , self.resource() ,  os.path.abspath(os.path.dirname(self.script)))
		debug_log.info(cmd)
		while count <= 10 : 
			qsub = "".join(popen(cmd))
			...
			debug_log.error("Exit :  qsub failed {0}".format(cmd))
			sys.exit(1)
