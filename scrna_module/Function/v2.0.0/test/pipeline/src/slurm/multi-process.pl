...
Usage: perl $program_name  <command_file>  
	...
	--sif <str>     set singularity image file
	--mount <str>     set singularity mount path
...
my %opts;
GetOptions(\%opts, "cpu:s","cmd!", "sif:s" , "mount:s" , "verbose!","help!" , "lines:s");
...
foreach my$i(@cmd){
	...
	if (exists $opts{sif}){
		...
		print OUT "singularity exec --bind $opts{mount} $opts{sif} /bin/bash <<EOF\n";
