...
class Script():
	...
	def qsub(self, f_out):
		...
		if self.qsub_P  and self.qsub_P != 'none' :
			cmd1 = 'cd {2} && export SGE_ROOT=/opt/gridengine && /opt/gridengine/bin/lx26-amd64/qsub -cwd -S /bin/sh -q {0.queue} -P {0.qsub_P} -l {1} {0.script}'.format(self , self.resource() ,  os.path.abspath(os.path.dirname(self.script)))
			cmd2 = 'cd {2} && qsub -cwd -S /bin/sh -q {0.queue} -P {0.qsub_P} -l {1} {0.script}'.format(self , self.resource() ,  os.path.abspath(os.path.dirname(self.script)))
		else:
			cmd1 = 'cd {2} && export SGE_ROOT=/opt/gridengine && /opt/gridengine/bin/lx26-amd64/qsub -cwd -S /bin/sh -q {0.queue}  -l {1} {0.script}'.format(self , self.resource() ,  os.path.abspath(os.path.dirname(self.script)))
			cmd2 = 'cd {2} && qsub -cwd -S /bin/sh -q {0.queue}  -l {1} {0.script}'.format(self , self.resource() ,  os.path.abspath(os.path.dirname(self.script)))
		debug_log.info(cmd1)
		debug_log.info(cmd2)
		while count <= 5 :
			qsub=''
			if count % 2 == 0 :
				try:
					qsub = "".join(popen(cmd1 , 3))
				except SystemExit :
					pass
			else:
				try:
					qsub = "".join(popen(cmd2 , 3))
				except SystemExit :
					pass
			...
			debug_log.error("Exit :  qsub failed {0}".format(cmd1))
			sys.exit(1)
	...
	def qrelease(self , db):
		...
		if len(qstat) > 0 : 
			...
			self.job_obj.qrls_time = time_current() #time.time()
	def qdel(self , db):
		cmd = ' export SGE_ROOT=/opt/gridengine && /opt/gridengine/bin/lx26-amd64/qdel {0.jobid}'.format(self)
